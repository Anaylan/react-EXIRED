...
const Checkout = () => {
	...
	const { cart, ticket, user, setUser, setCart, setTicket } =
	...
	const addOrder = async (data) => {
		...
		await axios
			.patch(`//localhost:8080/users/${user.id}`, {
				orders: [
					...user.orders,
					{
						...data,
	...
	return (
		<section className='checkout'>
			<div className='container'>
				<h2
					className='title'
					dangerouslySetInnerHTML={{ __html: t("checkout.title") }}
				/>
				<div className='basket__crumbs'>
					<Link className='basket__crumbs-home' to='/'>
						{t("checkout.link1")}
					</Link>
					-
					<NavLink className='basket__crumbs-cart' to='/checkout'>
						{t("checkout.link2")}
					</NavLink>
				</div>
				<form onSubmit={handleSubmit(addOrder)}>
					<div className='checkout__buyer'>
						<div className='checkout__buyer-form'>
							...
							<input
								{...register("name")}
								className='checkout__buyer-input'
								placeholder={t("checkout.buyerName")}
								type='text'
							/>
							<input
								{...register("email")}
								className='checkout__buyer-input'
								placeholder={t("checkout.buyerMail")}
								type='email'
							/>
							<InputMask
								...
								{...register("phone")}
								className='checkout__buyer-input'
								placeholder={t("checkout.buyerPhone")}
								type='tel'
							/>
						</div>
						<div className='checkout__buyer-left'>
							...
							<ul className='checkout__buyer-list'>
								<li className='checkout__buyer-item'>
									...
									<p className='checkout__buyer-info'>{t("checkout.total")}</p>
								</li>
								{cart.map((item, idx) => (
					...
					<div className='checkout__address'>
						<div className='checkout__address-form'>
							...
							/>
							<h3
								className='checkout__title checkout__title_com'
							...
							<textarea
								...
								cols='20'
								rows='5'
								className='checkout__address-comment'
								placeholder={t("checkout.comments")}
							/>
						</div>
						<div className='checkout__address-left'>
							<h3
								className='checkout__title'
								dangerouslySetInnerHTML={{ __html: t("checkout.payment") }}
							/>
							<div className='checkout__address-check'>
								<input className='checkout__address-checked' type='checkbox' />
								{t("checkout.cash")}
							</div>
							<button type='submit' className='checkout__address-btn'>
								{t("checkout.place")}
							</button>
						</div>
					</div>
