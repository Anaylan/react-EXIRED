...
const Basket = () => {
	...
	const { cart, deleteCart, setCart, ticket, setTicket } =
		useContext(CustomContext);

	const useTicket = (e) => {
		e.preventDefault();
		axios(`//localhost:8080/tickets?title=${e.target[0].value}`).then(
			({ data }) => {
				if (data.length) {
					setTicket(data);
				} else {
			...
			<div className='container'>
				...
				<div className='basket__info'>
					...
					<ul className='basket__info-list'>
						<li className='basket__info-item basket__info-item2'>
							{t("basket.size")}
						</li>
						<li className='basket__info-item basket__info-item2'>
							{t("basket.color")}
					...
					<>
						<div className='basket__ticket'>
							<form className='basket__ticket-left' onSubmit={useTicket}>
								<input
									className='basket__ticket-input'
									placeholder={t("basket.ticket")}
									type='text'
								/>
								<button type='submit' className='basket__ticket-btn'>
									{t("basket.useTicket")}
								</button>
								...
								{Array.isArray(ticket) && ticket.length ? (
									...
									<p className='basket__ticket-message'>{ticket}</p>
								) : (
									""
								)}
							</form>
							<button
								className='basket__ticket-btn'
								onClick={() => setCart([])}>
								{t("basket.updateCart")}
							</button>
						</div>
						<div className='basket__pay'>
							...
							<div className='basket__pay-btns'>
								...
								<Link to='/checkout'>
									<button className='basket__pay-btn2'>
										{t("basket.checkout")}
									</button>
								</Link>
							</div>
