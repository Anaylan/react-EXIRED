...
const Product = () => {
	...
	useEffect(() => {
		axios(`//localhost:8080/clothes/${params.id}`).then(({ data }) => {
			setProduct(data);
				...
				{product.title && (
					<>
						...
						<div className='contactHeader__links'>
							<Link className='contactHeader__link' to='/'>
								{t("contacts.crumbs.link1")}
							</Link>
							-
							<NavLink
								className='contactHeader__link'
								to='/shop'
								onClick={() => {
									setPage(1);
							...
							<div className='product__content-info'>
								{!product.priceSale ? (
									<>
										{sale ? (
											<input
												className='product__content-inputSale'
												value={saleCount}
										...
										{user.email === "admin@mail.ru" ? (
											<button
												className='product__content-btnSale'
												type='button'
												onClick={() => {
													if (sale) {
														axios
															.patch(`//localhost:8080/clothes/${product.id}`, {
																priceSale:
																	product.price -
																	(product.price / 100) * saleCount,
															})
															.then(() => {
																getAllClothes();
																setSaleCount(0);
								...
								<p className='product__content-price'>
									{product.priceSale ? (
										<>
											...
											<span className='product__content-price-sale'>
												{product.priceSale}{" "}
											</span>
										</>
									) : (
								...
								<ul className='product__content-sizes'>
									{product.size.map((item) => (
										<li
											...
											className={`product__content-size ${
												item === size ? "product__content-size_active" : ""
											} `}>
											{item}
										</li>
									))}
								...
								<ul className='product__content-sizes'>
									{product.colors.map((item) => (
										<li
											...
											className={`product__content-color ${
												item === color ? "product__content-color_active" : ""
											}`}
										/>
									))}
								</ul>
								...
								<div className='product__content-form'>
									...
									/>
									<button
										className='product__content-btn'
										style={{
											background: !product.inStock ? "grey" : "#6E9C9F",
										}}
										disabled={!product.inStock}
										type='button'
										onClick={() =>
