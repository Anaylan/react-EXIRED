...
const Profile = () => {
	...
	const changeUser = (data) => {
		axios.patch(`//localhost:8080/users/${user.id}`, data).then(({ data }) => {
			setUser(data);
	...
	const changePassword = (data) => {
		axios
			.patch(`//localhost:8080/users/${user.id}`, { password: data.password })
				...
				<div className='profile__header'>
					<div className='profile__crumbs'>
						...
						<div className='profile__crumbs-info'>
							...
							-
							<NavLink className='basket__crumbs-cart' to='/basket'>
								{t("profile.profile")}
							</NavLink>
						</div>
					</div>
					<div className='profile__btns'>
						<button
							onClick={() => setTab(1)}
							className={`profile__btn ${tab === 1 && "profile__btn-active"}`}>
							{t("profile.btn1")}
						</button>
						<button
							onClick={() => setTab(2)}
							className={`profile__btn ${tab === 2 && "profile__btn-active"}`}>
							{t("profile.btn2")}
						</button>
					</div>
				</div>
							...
							<div className='profile__orders' key={idx}>
								<ul className='profile__orders-list'>
									<li className='profile__orders-item'>
										<p className='profile__orders-info'>
											<span className='profile__orders-about'>
												{t("profile.name")}
											</span>{" "}
											{item.name}
										</p>
										...
										<p
											className='profile__orders-arrow'
									...
									<ul className='profile__orders-list'>
										{item.clothes.map((el) => (
											<li className='profile__orders-item ' key={el.id}>
												...
												<p className='profile__orders-info'>
													Название : <br />{" "}
													<span className='profile__orders-span'>
														{el.title}
													</span>{" "}
												</p>
												<p className='profile__orders-info'>
					...
					<div>
						<form
							...
							<div className='profile__change'>
								...
								<span>
									<span
										className='profile__content-write'
										onClick={() => setUserChange(!userChange)}>
										{userChange ? (
											<>
												<ImCross /> {t("profile.cancel")}{" "}
											</>
										) : (
											<>
							...
							<div className='profile__info'>
								<p className='profile__info-about'>
									<span className='profile__info-upper'>
										{t("profile.login")}
									</span>
									{userChange ? (
										<input
							...
							{userChange ? (
								<button className='profile__info-save' type='submit'>
									{t("profile.save")}
								</button>
							) : (
								""
						...
						<form
							...
							className='profile__content'>
							<div className='profile__change'>
								<h3 className='profile__content-title'>
									{t("profile.password")}
								</h3>
								<span>
									<span
